...
var RTCHub = function (username, onReady, onConnectionCallback) {
    ...
    this.establishConnection = function(destination) {
        // generate random connectionId for this connection
        var cid = btoa((new Date().getTime() + "-" + parseInt(Math.random()*1000)));

        var peerConnection = new RTCPeerConnection(servers);
        var dataChannel = peerConnection.createDataChannel(cid, {});
